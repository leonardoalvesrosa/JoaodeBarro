...
function Poster({ data, getData }) {
  ...
  const handleOpen = () => {
    ...
    return history.push(`/postagens/editar/${id}`);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <h2><Link to={`/postagens/editar/${data[0].poster.id}`}>{data[0].poster.poster_name[0].toUpperCase() + data[0].poster.poster_name.substr(1)}</Link></h2>
